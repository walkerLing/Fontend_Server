		...
		if (wait->next->device <= device) {
			...
			if (alloc(job->jobId, job->memory) == 1) {
				...
				if (processAl == FCFS || processAl == RR) {
					...
					//进程优先数调度，按照优先数从大到小排序，优先数相同按照进程号从小到大排序，优先数越大优先级越高
					while (node->next != NULL) {
...
int alloc(int job, int needSize) {
	LinkedFPT p = partHead;
	while (p != NULL) {//遍历空闲分区链
		if (p->data.state == FREE) {//若空闲区大小大于请求分配大小，则分配
	...
	if (p == NULL) {//若遍历完空闲分区链，没有找到合适的空闲区，则分配失败,返回0
		return 0;
	}
	else {//分配成功，返回1
		return 1;
...
void freePart(int job) {
	LinkedFPT p = partHead;
	while (p != NULL) {
		if (p->data.state == 1 && p->data.job == job) {
...
int main() {
	...
	init();	//初始化
	input();//输入作业
	while (readyJob->next != NULL || waitJob->next != NULL || nonarrivalJob->next != NULL) {//作业未全部完成时
		printf("\n\n当前时间为 %d:%02d\n", hour, minute);
		jobScheduling();
